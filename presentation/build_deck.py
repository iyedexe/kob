...
def slide(idx, title=None, kicker=None):
    ...
    if title:
        ...
        render(tbox(s, MX, Inches(0.64), CW, Inches(0.6)),
               [{"text": title, "size": 25, "bold": True, "color": PLUM}])
        fill(s.shapes.add_shape(MSO_SHAPE.RECTANGLE, MX, Inches(1.32), Inches(2.1), Pt(3)), PURPLE)
...
def code_box(s, x, y, w, h, lines, size=12.5):
    ...
    render(tf, [{"text": ln, "size": size, "color": CODEFG, "font": MONO, "after": 3, "ls": 1.28}
                for ln in lines])
    return box

def card(s, x, y, w, h, head, body, accent=PURPLE, head_size=15, body_size=12):
    ...
    # Head and body live in separate, fixed-position boxes so they can never overlap.
    render(tbox(s, x + Inches(0.32), y + Inches(0.22), w - Inches(0.54), Inches(0.4)),
           [{"text": head, "size": head_size, "bold": True, "color": PLUM}])
    render(tbox(s, x + Inches(0.32), y + Inches(0.74), w - Inches(0.54), h - Inches(0.92)),
           [{"text": body, "size": body_size, "color": INK, "ls": 1.25}])
...
render(tbox(s, Inches(0.9), Inches(1.35), Inches(11.5), Inches(1.95)),
       [{"text": "kob", "size": 96, "bold": True, "color": WHITE}])
render(tbox(s, Inches(0.95), Inches(3.2), Inches(11.6), Inches(1.4)),
       [{"text": "A tiny, self-discovering Parquet query server —", "size": 23, "color": ONDARK, "after": 5, "ls": 1.28},
        {"text": "served, read-only, as Apache Arrow — fast.", "size": 23, "color": ONDARK, "ls": 1.28}])
render(tbox(s, Inches(0.95), Inches(4.6), Inches(11.6), Inches(0.6)),
       [{"text": "Drop Parquet in a folder → a discoverable, filterable, blazing-fast Arrow API.",
         "size": 15, "italic": True, "color": ONDARKMUTE}])
...
# Slide 2 — Motivation
# ============================================================================
s = slide(2, "Serving Parquet shouldn't be hard — or slow", kicker="Motivation")
render(tbox(s, MX, Inches(1.55), CW, Inches(0.8)),
       [{"text": "A producer writes Parquet to a folder. You must serve it, read-only, to "
                 "Python / C# / C++ / Excel clients. Every usual option is overkill or slow:",
         "size": 15, "color": INK, "ls": 1.22}])
card(s, MX, Inches(2.55), Inches(3.75), Inches(2.15), "A warehouse / broker",
     "Heavy infrastructure in front of an already-queryable, columnar file format.", PURPLE)
card(s, Inches(4.78), Inches(2.55), Inches(3.75), Inches(2.15), "A hand-written API",
     "Declare a schema + catalog in code; redeploy on every new dataset or column.", LAV)
card(s, Inches(8.81), Inches(2.55), Inches(3.75), Inches(2.15), "A JSON / Protobuf API",
     "Pays the serialization tax — Arrow's FAQ: (de)serialization is 80–90% of compute.", MAGENTA)
render(tbox(s, MX, Inches(5.15), CW, Inches(1.4)),
       [{"runs": [{"text": "What we actually want:  ", "size": 16, "bold": True, "color": PLUM},
                  {"text": "self-discovering", "size": 16, "bold": True, "color": PURPLE},
                  {"text": " (no hardcoded schema)   ·   ", "size": 16, "color": INK},
                  {"text": "minimal", "size": 16, "bold": True, "color": PURPLE},
                  {"text": " (tiny footprint)   ·   ", "size": 16, "color": INK},
                  {"text": "blazing fast", "size": 16, "bold": True, "color": PURPLE},
                  {"text": " (no serialization tax).", "size": 16, "color": INK}], "ls": 1.3}])
...
render(tbox(s, MX, Inches(1.55), Inches(6.3), Inches(4.6)),
       [{"text": "Row vs. columnar is the whole story:", "size": 16, "bold": True, "color": PLUM, "after": 10},
        {"bullet": "dot", "text": "JSON & Protobuf are row-oriented. The sender encodes every value "
                                  "field-by-field; the receiver decodes every value and rebuilds columns.", "size": 14.5, "after": 8, "ls": 1.28},
        {"bullet": "dot", "text": "Cost scales with rows × cols — and grows super-linearly with result size.", "size": 14.5, "after": 8, "ls": 1.28},
        {"bullet": "dot", "text": "Arrow's columnar wire layout IS its in-memory layout. Reading a result is a "
                                  "header parse + buffer-pointer setup — essentially nothing to deserialize.", "size": 14.5, "after": 8, "ls": 1.28},
        {"bullet": "dot", "text": "Arrow Flight uses Protobuf only for its tiny control envelope (tickets), "
                                  "never for the bulk data.", "size": 14.5, "ls": 1.28}])
table(s, Inches(7.35), Inches(2.0), Inches(5.2),
      [["Format", "Layout", "Verdict"],
       ["JSON", "row, text", "slowest"],
       ["Protobuf", "row, binary", "binary ≠ fast"],
       ["Arrow IPC", "columnar", "near-zero"],
       ["Arrow Flight", "columnar + gRPC", "the default"]],
      col_w=[Inches(1.9), Inches(1.9), Inches(1.4)])
render(tbox(s, Inches(7.35), Inches(4.7), Inches(5.2), Inches(1.7)),
       [{"text": "“(De)serialization can represent 80–90% of computing costs.”", "size": 13, "italic": True, "color": PLUM, "after": 12, "ls": 1.25},
...
render(tbox(s, MX, Inches(1.5), Inches(6.2), Inches(4.5)),
       [{"text": "One straight line, folder → Arrow stream:", "size": 15, "bold": True, "color": PLUM, "after": 8},
        ...
        {"bullet": "dot", "text": "server — the kob entry point: Flight + a Swagger HTTP control plane, one process, one engine.", "size": 13.5, "after": 6, "ls": 1.28},
        {"bullet": "dot", "text": "demos — Arrow-over-HTTP / JSON / Protobuf servers, only to demonstrate the alternatives.", "size": 13.5, "after": 6, "ls": 1.28},
        {"bullet": "dot", "text": "tools — sample-data generator, reference client, benchmark.", "size": 13.5, "ls": 1.28}])
...
render(tbox(s, MX, Inches(1.55), Inches(6.0), Inches(4.6)),
       [{"bullet": "dot", "runs": [{"text": "Arrow wins by 1–3 orders of magnitude", "size": 15, "bold": True, "color": PLUM},
        ...
        {"bullet": "dot", "runs": [{"text": "Binary ≠ fast.", "size": 15, "bold": True, "color": PLUM},
                                   {"text": " Protobuf is binary and still 5–56× slower — it must parse every value.", "size": 15, "color": INK}], "after": 10, "ls": 1.28},
        {"bullet": "dot", "runs": [{"text": "The gap grows with result size", "size": 15, "bold": True, "color": PLUM},
                                   {"text": " — ~150× on 29k rows → ~1750× on 1.75M rows.", "size": 15, "color": INK}], "after": 10, "ls": 1.28},
        {"bullet": "dot", "runs": [{"text": "JSON is a bandwidth hog too", "size": 15, "bold": True, "color": PLUM},
                                   {"text": " — 8× heavier on the wire than Arrow + zstd.", "size": 15, "color": INK}], "after": 10, "ls": 1.28},
        {"bullet": "dot", "runs": [{"text": "Same engine throughout.", "size": 15, "bold": True, "color": PLUM},
                                   {"text": " Only the wire format changes — so the wire is the cause.", "size": 15, "color": INK}], "ls": 1.28}])
s.shapes.add_picture(str(SPD), Inches(7.0), Inches(1.9), width=Inches(5.6))
render(tbox(s, Inches(7.0), Inches(4.65), Inches(5.6), Inches(0.4)),
...
render(tbox(s, Inches(10.0), Inches(1.9), Inches(2.6), Inches(4.6)),
       [{"text": "1.75M rows", "size": 15, "bold": True, "color": PLUM, "after": 2},
        ...
        {"text": "Same result, ~1750× apart.", "size": 13, "italic": True, "color": PLUM, "ls": 1.25, "after": 10},
        {"text": "Pushdown happens before serialization — and compounds with picking a fast wire.", "size": 11.5, "color": GRAY, "ls": 1.25}])
render(tbox(s, Inches(1.35), Inches(5.75), Inches(8.4), Inches(0.85)),
       [{"text": "Medium dataset · Apple M4 · localhost loopback (understates Arrow's win on a real network).",
...
render(tbox(s, MX, Inches(1.5), Inches(6.1), Inches(4.6)),
       [{"text": "A runnable notebook spins up all four transports, runs the SAME query over each, "
        ...
        {"bullet": "dot", "text": "Arrow Flight  ·  Arrow-over-HTTP  ·  REST/JSON  ·  gRPC/Protobuf", "size": 14, "bold": True, "color": PLUM, "after": 8, "ls": 1.28},
        {"bullet": "dot", "runs": [{"text": "Result:  ", "size": 14, "color": INK},
                                   ...
                                   {"text": " — Flight fastest by far.", "size": 14, "color": INK}], "after": 8, "ls": 1.28},
        {"bullet": "dot", "text": "Discovery, filtering and serialization are all kob's job — zero schema config.", "size": 14, "color": INK, "ls": 1.28}])
...
render(tbox(s, Inches(0.95), Inches(2.0), Inches(11.6), Inches(4.0)),
       [{"bullet": "dot", "runs": [{"text": "Use Arrow on the wire, default to Flight. ", "size": 18, "bold": True, "color": ACCENTL},
                                   {"text": "Not JSON (150–1750× slower, 8× heavier), not Protobuf (5–56× slower).", "size": 18, "color": ONDARK}], "after": 14, "ls": 1.28},
        {"bullet": "dot", "runs": [{"text": "Self-discovering beats configured. ", "size": 18, "bold": True, "color": ACCENTL},
                                   {"text": "Point kob at a folder; new data appears live, no restart, no schema.", "size": 18, "color": ONDARK}], "after": 14, "ls": 1.28},
        {"bullet": "dot", "runs": [{"text": "Minimal & safe by construction. ", "size": 18, "bold": True, "color": ACCENTL},
                                   {"text": "Four deps, ~750-line core, read-only, parameterised, sandboxed.", "size": 18, "color": ONDARK}], "after": 14, "ls": 1.28},
        {"bullet": "dot", "runs": [{"text": "It's the wire, not the engine. ", "size": 18, "bold": True, "color": ACCENTL},
                                   {"text": "Hold DuckDB constant and the format alone moves latency by 1750×.", "size": 18, "color": ONDARK}], "ls": 1.28}])
